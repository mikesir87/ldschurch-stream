...
3. Creates a new stream event for the next Sunday sacrament meeting
  ...
  - The backend, using its YouTube integration, will then create the YouTube Live event and a stream key unique to the unit and associate the stream URL and stream key with this event. 
...
On the day of the event, they will be prompted with a form to provide their name and number of attendees, after which they will be forwarded to the stream (even if it isn't live yet). If the same family opens the app multiple times and submits the name multiple times, the dashboard does not try to consolidate the submissions.
