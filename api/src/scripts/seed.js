...
const seedData = async () => {
  try {
    ...
    // Create test units
    let unit1 = await Unit.findOne({ subdomain: 'test-unit' });
    if (!unit1) {
      ...
      console.log('Created test unit:', unit1.name);
    }

    let unit2 = await Unit.findOne({ subdomain: 'second-unit' });
    if (!unit2) {
      ...
      console.log('Created second unit:', unit2.name);
    }
    ...
    const existingSpecialist = await User.findOne({ email: 'specialist@example.com' });
    if (!existingSpecialist) {
      ...
      console.log('Specialist user already exists:', existingSpecialist.email);
    }
    ...
    for (const unit of units) {
      ...
      if (existingEvents === 0) {
        ...
        for (const event of pastEvents) {
          ...
          const selectedNames = attendeeNames
        ...
        console.log(
          `Created ${pastEvents.length} past events with attendance data for ${unit.name}`
        );
      } else {
        console.log(`Stream events already exist for ${unit.name}, skipping event creation`);
      }
    }
